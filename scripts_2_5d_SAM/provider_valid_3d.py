...
class Provider_valid(Dataset):
    def __init__(self, cfg, valid_data=None, num_z=18, test=False, test_split=None, stage=None):
        # basic settings
        self.cfg = cfg
        ...
        elif valid_dataset_name == 'fib1':
            self.sub_path = 'fib1'
            self.train_datasets = ['fib1_inputs.h5']
            self.train_labels = ['fib1_labels.h5']
        elif cfg.DATA.dataset_name == 'fib2':
            self.sub_path = 'fib2'
            self.train_datasets = ['fib2_inputs.h5']
            self.train_labels = ['fib2_labels.h5']
        ...
        for k in range(len(self.train_datasets)):
            ...
            elif valid_dataset_name == 'zebrafinch':  # 20240422
        ...
        if cfg.MODEL.model_type == 'mala':
            if 'zebrafinch' in valid_dataset_name:
            ...
            if 'fib' in valid_dataset_name:
                padding_xy = 20
                num_xy = 6
            elif 'cremiA' in valid_dataset_name or 'cremiB' in valid_dataset_name or 'cremiC' in valid_dataset_name or 'wafer4' in valid_dataset_name:
                padding_xy = 15
                num_xy = 15
            elif 'zebrafinch' in valid_dataset_name:
                padding_xy = 16
                num_xy = 8
                if hasattr(cfg.DATA, 'raw_3d') and cfg.DATA.raw_3d is True:
                    padding_xy = 16
            ...
            elif self.dataset[0].shape[0] == 200:
                self.stride = [10, 80, 80]
                self.valid_padding = [4, padding_xy, padding_xy]
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.DATA.dataset_name == 'zebrafinch':
            self.raw_data_shape = list(self.dataset[pos_data].shape)
