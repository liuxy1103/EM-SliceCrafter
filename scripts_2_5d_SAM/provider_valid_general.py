...
class Provider_valid(Dataset):
    def __init__(self, cfg, valid_data=None, test=False, test_split=None, direc='x'):
        ...
        if valid_dataset_name == 'cremiA':
            ...
            # Handle vol_idx if it exists
            try:
                if hasattr(cfg.DATA, 'vol_idx') and cfg.DATA.vol_idx is not None:
                    self.train_datasets = [self.train_datasets[int(cfg.DATA.vol_idx)]]
                    self.train_labels = [self.train_labels[int(cfg.DATA.vol_idx)]]
            except AttributeError:
                pass  # Skip if vol_idx doesn't exist
        elif valid_dataset_name == 'zebrafinch_test':
            self.sub_path = 'zebrafinch/test'
            self.folder_name = os.path.join(cfg.DATA.data_folder, self.sub_path)
            ...
            try:
                ...
                pass  # Skip if vol_idx doesn't exist
        elif valid_dataset_name == 'zebrafinch_test_128':
            self.sub_path = 'zebrafinch/test_128'
            self.folder_name = os.path.join(cfg.DATA.data_folder, self.sub_path)
        ...
        for k in range(len(self.train_datasets)):
            ...
            elif valid_dataset_name == 'wafer4':  # 20240422
                print('wafer4 data')
                data = np.where(data > 255, 255, data.astype('uint8'))
                data = data[-self.test_split:]
            elif valid_dataset_name == 'wafer36-2':  # 20240422
                print('whole wafer36-2 data')
                data = np.where(data > 255, 255, data.astype('uint8'))
            elif valid_dataset_name == 'zebrafinch':  # 20240425
                print('whole zebrafinch/val data')
                data = np.where(data > 255, 255, data.astype('uint8'))
            elif valid_dataset_name == 'zebrafinch_test' or valid_dataset_name == 'zebrafinch_test_128':   
                print('whole zebrafinch/test data')
                data = np.where(data > 255, 255, data.astype('uint8'))
            else:
                data = data[-self.test_split:]
            ...
            if valid_dataset_name == 'ac3':
                ...
                print('whole wafer36-2 label')
            elif valid_dataset_name == 'zebrafinch':  # 20250425
                print('whole zebrafinch/val label')  
            elif valid_dataset_name == 'zebrafinch_test' or valid_dataset_name == 'zebrafinch_test_128':   
                print('whole zebrafinch/test label')  
            else:
                label = label[-self.test_split:]  
        ...
        # Use actual dataset size instead of test_split
        if self.valid_dataset_name == 'zebrafinch' or self.valid_dataset_name == 'zebrafinch_test' or self.valid_dataset_name == 'zebrafinch_test_128':
            self.num_per_dataset = self.dataset[0].shape[0]  # Use actual size of the dataset
        else:
    ...
    def __getitem__(self, index):
        ...
        try:
            if self.cfg.DATA.vol_idx is None:
                if self.valid_dataset_name == 'zebrafinch' or self.valid_dataset_name == 'zebrafinch_test' or self.valid_dataset_name == 'zebrafinch_test_128': 
                    index = index % self.num_per_dataset
                    # Handle boundary case
                    if index >= self.dataset[pos_data].shape[0]:
                        index = self.dataset[pos_data].shape[0] - 1
        except AttributeError:
            if self.valid_dataset_name == 'zebrafinch' or self.valid_dataset_name == 'zebrafinch_test' or self.valid_dataset_name == 'zebrafinch_test_128': 
                index = index % self.num_per_dataset
            pass
        ...
        if self.mode == 'x-y-z-2':
            ...
            #     if index == 150:
            #         img2 = self.dataset[pos_data][index-1].copy()
            #         lb2 = self.labels[pos_data][index-1].copy()
            #     else:
