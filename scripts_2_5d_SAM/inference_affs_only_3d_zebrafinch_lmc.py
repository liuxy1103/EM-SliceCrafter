...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='3d_segmamba_zebrafinch_pure_inference', help='path to config file')
    parser.add_argument('-mn', '--model_name', type=str, default='2025-06-01--02-29-20_3d_segmamba_zebrafinch_pure')
    parser.add_argument('-id', '--model_id', type=int, default=195000)
    ...
    valid_provider = Provider_valid(cfg, valid_data=args.mode, test_split=args.test_split)  # 针对zebrafinch_test_128，test_split不影响zebrafinch系列数据集的读取
    ...
    for k, batch in enumerate(val_loader, 0):
        ...
        if (k + 1) % volume_size == 0:
            ...
            if hasattr(cfg.TEST, 'if_seg') and cfg.TEST.if_seg:
                ...
                print('the max id = %d' % np.max(segmentation_lmc))
                f = h5py.File(os.path.join(vol_out_dir, 'seg_lmc.hdf'), 'w')
