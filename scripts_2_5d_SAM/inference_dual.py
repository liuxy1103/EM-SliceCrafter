...
from CoDetectionCNN import CoDetectionCNN, CoDetectionCNN_extra_SAM2
...
def build_model_SAM2(cfg, writer=None):
...
def build_model_extra_SAM2(cfg, writer=None):
    print('Building model on ', end='', flush=True)
    t1 = time.time()
    device = torch.device('cuda:0')
    model = CoDetectionCNN_extra_SAM2(n_channels=cfg.MODEL.input_nc,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='cremi_2D_bs32_inference', help='path to config file')
    parser.add_argument('-mn', '--model_name', type=str, default='/h3cstore_ns/hyshi/scripts_2_5d/models_SAM/2024-11-06--21-32-03_ac4_2D_mse_bs32_ps128_dual0.1')
    parser.add_argument('-id', '--model_id', type=int, default=115000)
    parser.add_argument('-m', '--mode', type=str, default='cremiA') # default='wafer4'
    ...
    out_path = os.path.join('/h3cstore_ns/hyshi/scripts_2_5d/inference', trained_model, args.mode)
    ...
    device = torch.device('cuda:0')
    # model1 = CoDetectionCNN(n_channels=cfg.MODEL.input_nc,
    #                     n_classes=cfg.MODEL.output_nc,
    #                     filter_channel=cfg.MODEL.filter_channel,
    #                     sig=cfg.MODEL.if_sigmoid).to(device)
    model1 = build_model_extra_SAM2(cfg)

    model2 = build_model_SAM2(cfg)

    ckpt_path = os.path.join(trained_model, 'model-%06d.ckpt' % args.model_id)
    ...
    for k, data in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            if inputs.shape[-1] == 1250:
                ...
                embedding2_1 = model2(x_in1)['backbone_fpn']
                embedding2_2 = model2(x_in2)['backbone_fpn']
                embedding1, embedding2 = model1(inputs, embedding2_1, embedding2_2)
