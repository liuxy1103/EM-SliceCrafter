...
from model_channelShift import StepByStepUpscaler,StepByStepUpscaler2,StepByStepUpscaler3,StepByStepUpscaler4
...
def loop(cfg, train_provider, valid_provider, model, shift_model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        embedding1,embedding2 = shift_model(embedding2_1,embedding2_2)
        # embedding2 = shift_model(embedding2_2)
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        model = build_model_SAM2(cfg, writer)
        shift_model = build_model_shift(cfg, writer)
        optimizer = torch.optim.Adam(shift_model.parameters(), lr=cfg.TRAIN.base_lr, betas=(0.9, 0.999),
                                 eps=0.01, weight_decay=1e-6, amsgrad=True)
        model, optimizer, init_iters = resume_params(cfg, model, optimizer, cfg.TRAIN.resume)
        loop(cfg, train_provider, valid_provider, model, shift_model, nn.L1Loss(), optimizer, init_iters, writer)
        writer.close()
    else:
        pass
    print('***Done***')