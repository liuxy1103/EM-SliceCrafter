...
if __name__ == "__main__":
    ...
    parser.add_argument('-mn', '--model_name', type=str, default='2023-10-08--12-22-38_2d_bs16_ps256_loss0.0_slice1.0_cross0_interaction0_3d_z16_h160_noft_lr_ratio1')
    parser.add_argument('-id', '--model_id', type=int, default=179000)
    ...
    out_path = os.path.join('../inference_vis', trained_model, args.mode)
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    img_folder = 'affs_'+str(args.model_id)
    out_affs = os.path.join(out_path, img_folder)
    emb_folder = 'embs_'+str(args.model_id)
    emb_folder = os.path.join(out_path, img_folder)
    ...
    for k, data in enumerate(val_loader, 0):
        ...
        emb = embedding1[0].cpu().numpy()
        emb_pca = embedding_pca(emb)
        emb_pca = np.transpose(emb_pca, (1,2,0))
        cv2.imwrite(os.path.join(emb_folder, str(k).zfill(4)+'_emb.png'), emb_pca)
        pbar.update(1)
    pbar.close()
