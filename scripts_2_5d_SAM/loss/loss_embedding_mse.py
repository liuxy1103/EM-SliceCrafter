...
def embedding_loss_norm_multi(embedding, target, weightmap, criterion, affs0_weight=1, shift=1, fill=True):
    '''
    shift= [[-1,0,0],
            [0,-1,0],
            [0,0,-1],
    ...
    for i in range(len(shift)):
        ...
        if z_sf<=0 and y_sf<=0 and x_sf<=0:
            affs_tmp = torch.sum(embedding[:, :, -z_sf:, -y_sf:, -x_sf:]*embedding[:, :, :D+z_sf, :H+y_sf, :W+x_sf], dim=1, keepdim=True)
            affs[:, i:i+1, -z_sf:, -y_sf:, -x_sf:] = affs_tmp
            loss_tmp = criterion(affs_tmp, target[:, i:i+1, -z_sf:, -y_sf:, -x_sf:], weightmap[:, i:i+1, -z_sf:, -y_sf:, -x_sf:])
        elif z_sf>0:
            affs = torch.sum(embedding[:, :, z_sf:, -y_sf:, -x_sf:]*embedding[:, :, :D-z_sf, :H+y_sf, :W+x_sf], dim=1, keepdim=True)
            affs[:, i:i+1, :D-z_sf, -y_sf:, -x_sf:] = affs_tmp
            loss_tmp = criterion(affs_tmp, target[:, i:i+1, :D-z_sf, -y_sf:, -x_sf:], weightmap[:, i:i+1, :D-z_sf, -y_sf:, -x_sf:])
        elif y_sf>0:
            affs_tmp = torch.sum(embedding[:, :, -z_sf:, y_sf:, -x_sf:]*embedding[:, :, :D+z_sf, :H-y_sf, :W+x_sf], dim=1, keepdim=True)
            affs[:, i:i+1, -z_sf:, :H-y_sf, -x_sf:] = affs_tmp
            loss_tmp = criterion(affs_tmp, target[:, i:i+1, -z_sf:, :H-y_sf, -x_sf:], weightmap[:, i:i+1, -z_sf:, :H-y_sf, -x_sf:])
        elif x_sf>0:
            affs_tmp = torch.sum(embedding[:, :, -z_sf:, -y_sf:, x_sf:]*embedding[:, :, :D+z_sf, :H+y_sf, :W-x_sf], dim=1, keepdim=True)
            affs[:, i:i+1, -z_sf:, -y_sf:, :W-x_sf] = affs_tmp
            loss_tmp = criterion(affs_tmp, target[:, i:i+1, -z_sf:, -y_sf:, :W-x_sf], weightmap[:, i:i+1, -z_sf:, -y_sf:, :W-x_sf])
        if i <= 2 :
...
def embedding_loss_norm_multi_relu(embedding, target, weightmap, criterion, affs0_weight=1, shift=1, fill=True):
    '''
    shift= [[-1,0,0],
            [0,-1,0],
            [0,0,-1],
...
def embedding_single_offset_loss_intera(z, embedding, order, shift, target, weightmap, criterion):
    B, C, D, H, W = embedding.shape
    order_shift = order % 3
    if order_shift == 0:
        affs_temp = torch.sum(embedding[:, :, shift:, :, :]*embedding[:, :, :D-shift, :, :], dim=1, keepdim=True)
...
def embedding_loss_norm5_intera(z, embedding, target, weightmap, criterion, affs0_weight=1, shift=1, fill=True):
    '''
    Based on embedding_loss_norm4, but compute each channel separately 
    '''
    embedding = F.normalize(embedding, p=2, dim=1)
    ...
    for order, shift in enumerate(shifts):
        ...
        if z >= D-shift:
            ...
            affs_temp = torch.sum(embedding[:, :, shift:, :, :]*embedding[:, :, :D-shift, :, :], dim=1, keepdim=True)
            loss_temp = criterion(affs_temp[:,:,z], target[:, order:order+1, shift:, :, :][:,:,z], weightmap[:, order:order+1, shift:, :, :][:,:,z])
        else:
            loss_temp = 0
        loss += loss_temp
    return loss
