            ...
            if iters % cfg.TRAIN.save_freq == 0 or iters == 1:
                ...
                for k, batch in enumerate(dataloader, 0):
                    ...
                    if (k + 1) % volume_size == 0 or k == len(dataloader) - 1:
                        pbar.set_description(f"处理了volume {volume_idx}")
                ...
                for vol_idx in range(len(all_volume_preds)):
                    if len(all_volume_preds[vol_idx]) == 0:
                        print(f"警告: Volume {vol_idx} 没有预测结果")
                        continue
                    ...
                    # 计算MSE
                    whole_mse = np.sum(np.square(volume_pred - gt_affs)) / np.size(gt_affs)
                    all_metrics['mse'].append(whole_mse)
                    ...
                    all_metrics['bce'].append(whole_bce)
                    ...
                    pred_flat = (1 - volume_pred_binary.astype(np.uint8)).flatten()
                    whole_f1 = f1_score(gt_flat, pred_flat)
                    all_metrics['f1'].append(whole_f1)

                    # 保存可视化结果
                    save_path_vol = os.path.join(cfg.valid_path, f'volume_{vol_idx}')
                    os.makedirs(save_path_vol, exist_ok=True)
                    show_affs_whole2(iters, volume_pred, gt_affs, save_path_vol, cfg.TRAIN.mode)
                    ...
                    if cfg.TRAIN.if_seg:
                        ...
                        all_metrics['voi_waterz'].append(voi_sum_waterz)
                        all_metrics['arand_waterz'].append(arand_waterz)
                        # all_metrics['voi_lmc'].append(voi_sum_lmc)
                        # all_metrics['arand_lmc'].append(arand_lmc)
                    else:
                        all_metrics['voi_waterz'].append(0.0)
                        all_metrics['arand_waterz'].append(0.0)
                        # all_metrics['voi_lmc'].append(0.0)
                        # all_metrics['arand_lmc'].append(0.0)
                    
                    # 输出当前volume的所有指标
                    volume_metrics_str = (f'model-{iters}, volume-{vol_idx}, '
                                         f'loss={avg_volume_loss:.6f}, '
                                         f'MSE={whole_mse:.6f}, '
                                         f'BCE={whole_bce:.6f}, '
                                         f'F1={whole_f1:.6f}, '
                                         f'VOI-waterz={voi_sum_waterz:.6f}, '
                                         f'ARAND-waterz={arand_waterz:.6f}, '
                                         # f'VOI-lmc={voi_sum_lmc:.6f}, '
                                         # f'ARAND-lmc={arand_lmc:.6f}'
                                         )
                    
                    print(volume_metrics_str)
                    f_valid_volumes_txt.write(volume_metrics_str + '\n')
                ...
                writer.add_scalar('valid/voi_waterz', avg_metrics['voi_waterz'], iters)
                writer.add_scalar('valid/arand_waterz', avg_metrics['arand_waterz'], iters)
                # writer.add_scalar('valid/voi_lmc', avg_metrics['voi_lmc'], iters)
                # writer.add_scalar('valid/arand_lmc', avg_metrics['arand_lmc'], iters)

                # 打印并记录所有volume的平均结果
                avg_metrics_str = (f'model-{iters}, AVERAGE: '
                                  f'valid-loss={avg_metrics["loss"]:.6f}, '
                                  f'MSE-loss={avg_metrics["mse"]:.6f}, '
                                  f'BCE-loss={avg_metrics["bce"]:.6f}, '
                                  f'F1-score={avg_metrics["f1"]:.6f}, '
                                  f'VOI-waterz={avg_metrics["voi_waterz"]:.6f}, '
                                  f'ARAND-waterz={avg_metrics["arand_waterz"]:.6f}, '
                                #   f'VOI-lmc={avg_metrics["voi_lmc"]:.6f}, '
                                #   f'ARAND-lmc={avg_metrics["arand_lmc"]:.6f}'
                                  )
                
                print(avg_metrics_str)
                f_valid_txt.write(avg_metrics_str)
                f_valid_txt.write('\n')
                f_valid_txt.flush()
