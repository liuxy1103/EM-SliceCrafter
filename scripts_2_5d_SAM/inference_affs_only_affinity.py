...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='2d_SAM_MEC_slice1.0_aff0.0_cross1.0_interaction1.0_Lsam0.1_L3d1.0_interation0.1_v4', help='path to config file')
    parser.add_argument('-mn', '--model_name', type=str, default='2024-12-31--11-27-02_2d_SAM_cremiC_slice1.0_aff1.0_cross1.0_interaction0.1_Lsam0.1_L3d1.0_interation1.0_v4')
    parser.add_argument('-id', '--model_id', type=int, default=281000)
    ...
    out_path = os.path.join('/h3cstore_ns/hyshi/CAD_SAM2/inference', trained_model, args.mode)
    ...
    ckpt_path = os.path.join('/h3cstore_ns/liuxyu/models', trained_model, 'model-%06d.ckpt' % args.model_id)
    ...
    for k, data in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            ...
            if inputs.shape[-1] == 1250:
                ...
                # print(f"Actual input shape: {inputs.shape}")
                # macs, params = get_model_complexity_info(model, (inputs.shape[1], inputs.shape[2], inputs.shape[3]), print_per_layer_stat=False)
