...
class Train(Dataset):
	def __init__(self, cfg):
		...
		self.if_sparse = cfg.DATA.if_sparse
		# basic settings
		# the input size of network
		if not cfg.MODEL.model_type == 'mala':
		...
		elif cfg.DATA.dataset_name == 'fib1':
			self.sub_path = 'fib1'
		...
		print('Current sparse label volume:', self.train_labels)
	...
	def __getitem__(self, index):
		...
		if self.shift_channels is None:
			if self.if_sparse:
				...
				if_bg = True
			if self.output_nc == 3:
				lb_affs = seg_to_aff(lb,if_bg=if_bg).astype(np.float32)
		...
		return imgs, lb, lb_affs, weightmap
