...
def loop(cfg, train_provider, valid_provider, model1, model2, shift_model, criterion, optimizer, iters, writer, args):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        embedding2_1 = shift_model(embedding2_1)
        embedding2_2 = shift_model(embedding2_2)
        ...
        loss = loss_aff_consistency + loss_aff
        loss.backward()
        ##############################
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            if iters == 1:
                logging.info('step %d, loss = %.6f, loss_aff = %.6f, loss_ctc = %.6f (wt: *1, lr: %.8f, et: %.2f sec, rd: %.2f min)'
                            % (iters, sum_loss * 1,sum_aff_loss,sum_ctc_loss, current_lr, sum_time,
                            (cfg.TRAIN.total_iters - iters) / cfg.TRAIN.display_freq * np.mean(np.asarray(rcd_time)) / 60))
                writer.add_scalar('loss', sum_loss * 1, iters)
            else:
                logging.info('step %d, loss = %.6f, loss_aff = %.6f, loss_ctc = %.6f (wt: *1, lr: %.8f, et: %.2f sec, rd: %.2f min)'
                            % (iters, sum_loss / cfg.TRAIN.display_freq * 1, sum_aff_loss / cfg.TRAIN.display_freq * 1,
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        loop(cfg, train_provider, valid_provider, model1, model2, shift_model, nn.L1Loss(), optimizer, init_iters, writer, args)
        writer.close()
    else:
        pass
    print('***Done***')