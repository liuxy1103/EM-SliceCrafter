...
from utils.show_3d import show_affs, show_affs_whole,show_affs2
...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        # loss, pred = embedding_loss_norm_multi(embedding, target, weightmap, criterion, affs0_weight=cfg.TRAIN.affs0_weight,shift=cfg.DATA.shift_channels)
        ...
        if iters % cfg.TRAIN.valid_freq == 0 or iters == 1:
            ...
            show_affs2(iters, inputs[:,0,:2], pred[:,:3,1], target[:,:3,1], cfg.cache_path)
        # valid
        if cfg.TRAIN.if_valid:
