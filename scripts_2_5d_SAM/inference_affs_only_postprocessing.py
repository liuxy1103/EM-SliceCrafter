...
if __name__ == "__main__":
    ...
    parser.add_argument('-id', '--model_id', type=int, default=400000)
    ...
    # model.eval()
    # loss_all = []
    f_txt = open(os.path.join(out_affs, 'scores.txt'), 'a')
    ...
    # save
    print('read gt affs...')
    f = h5py.File(os.path.join(out_affs, 'affs_gt.hdf'), 'r')
    gt_affs = f['main'][:]
    print('the shape of affs:', gt_affs.shape)
    f.close()


    print('read gt segmentation')
    f = h5py.File(os.path.join(out_affs, 'seg_gt.hdf'), 'r')
    gt_seg = f['main'][:]
    f.close()
