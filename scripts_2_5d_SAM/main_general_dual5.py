...
from CoDetectionCNN import CoDetectionCNN, CoDetectionCNN_Add_SAM2, CoDetectionCNN_extra_SAM2
...
from model_channelShift import StepByStepUpscaler
...
def build_model_extra_SAM2(cfg, writer):
    print('Building model on ', end='', flush=True)
    t1 = time.time()
    device = torch.device('cuda:0')
...
def build_model_shift(cfg, writer):
    ...
    t1 = time.time()
    device = torch.device('cuda:0')
    shift_model = StepByStepUpscaler().to(device=device)
    # cuda_count = torch.cuda.device_count()
    cuda_count = 1
    model = shift_model
    if cuda_count > 1:
...
def loop(cfg, train_provider, valid_provider, model1, model2, criterion, optimizer, iters, writer, args):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dual', type=str, default='false', help='training mode, true for dual')
    ...
    if args.mode == 'train':
        ...
        logging.info('task description: 2D + SAM2 feature fusion: extra feature')

        model1 = build_model_extra_SAM2(cfg, writer)
        ...
        loop(cfg, train_provider, valid_provider, model1, model2, nn.L1Loss(), optimizer, init_iters, writer, args)
        writer.close()
    else:
        pass
    print('***Done***')