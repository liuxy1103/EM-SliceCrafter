...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--cfg', type=str, default='2d_SAM_zebrafinch_1.0_1.0_0.1_1.0_inference', help='path to config file')
    # parser.add_argument('-c', '--cfg', type=str, default='2d_CAD_zebrafinch_inference', help='path to config file')
    parser.add_argument('-mn', '--model_name', type=str, default='2025-05-30--15-44-18_2d_SAM_zebrafinch_1.0_1.0_0.1_1.0')
    ...
    parser.add_argument('-id', '--model_id', type=int, default=306000)
    ...
    img_folder = 'affs_'+str(args.model_id)+'_lmc'
    ...
    for vol_idx in range(len(all_volume_preds)):
        ...
        if hasattr(cfg.TEST, 'if_seg') and cfg.TEST.if_seg:
            ...
            # LMC分割
            print('Running LMC segmentation...')
            seg_lmc = mc_baseline(volume_pred)
            seg_lmc = relabel(seg_lmc).astype(np.uint64)
            ...
            all_metrics['voi_lmc'].append(voi_sum_lmc)
            all_metrics['voi_split_lmc'].append(voi_split_lmc)
            all_metrics['voi_merge_lmc'].append(voi_merge_lmc)
            all_metrics['arand_lmc'].append(arand_lmc)
            
            # 保存LMC分割结果
            vol_seg_lmc_path = os.path.join(vol_out_dir, 'seg_lmc.hdf')
            f = h5py.File(vol_seg_lmc_path, 'w')
            ...
            all_metrics['voi_lmc'].append(0)
            all_metrics['voi_split_lmc'].append(0)
            all_metrics['voi_merge_lmc'].append(0)
            all_metrics['arand_lmc'].append(0)
        ...
        volume_metrics_str = (f'volume-{vol_idx}, '
                             ...
                             f'VOI-LMC={all_metrics["voi_lmc"][-1]:.6f}, '
                             f'VOI-split-LMC={all_metrics["voi_split_lmc"][-1]:.6f}, '
                             f'VOI-merge-LMC={all_metrics["voi_merge_lmc"][-1]:.6f}, '
                             f'ARAND-LMC={all_metrics["arand_lmc"][-1]:.6f}')
...
#     # 如果需要进行分割评估，对整个数据集执行LMC分割
#     if hasattr(cfg.TEST, 'if_seg') and cfg.TEST.if_seg:
#         # 取前3个通道用于分割评估
#         seg_all_volumes_pred = all_volumes_pred[:, :3] if all_volumes_pred.shape[1] >= 3 else all_volumes_pred
#         seg_all_volumes_pred = np.ascontiguousarray(seg_all_volumes_pred, dtype=np.float32)
#         seg_all_volumes_pred = np.transpose(seg_all_volumes_pred, (1, 0, 2, 3))  # 转换回(C, Z, H, W)格式
        
#         # 使用LMC分割整个数据集
#         print('Running LMC segmentation on the whole dataset...')
#         seg_lmc_all = mc_baseline(seg_all_volumes_pred)
#         seg_lmc_all = relabel(seg_lmc_all).astype(np.uint64)
#         print('the max id = %d' % np.max(seg_lmc_all))
        
#         # 保存整个数据集的LMC分割结果
#         f = h5py.File(os.path.join(out_affs, 'seg_lmc.hdf'), 'w')
#         f.create_dataset('main', data=seg_lmc_all, dtype=seg_lmc_all.dtype, compression='gzip')
#         f.close()
        
#         # 计算整个数据集的LMC分割指标
#         arand_lmc_all = adapted_rand_ref(gt_seg, seg_lmc_all, ignore_labels=(0))[0]
#         voi_split_lmc_all, voi_merge_lmc_all = voi_ref(gt_seg, seg_lmc_all, ignore_labels=(0))
#         voi_sum_lmc_all = voi_split_lmc_all + voi_merge_lmc_all
        
#         # 输出并记录整个数据集的LMC分割指标
#         lmc_all_metrics_str = (f'WHOLE DATASET LMC METRICS: '
#                               f'VOI-LMC={voi_sum_lmc_all:.6f}, '
#                               f'VOI-split-LMC={voi_split_lmc_all:.6f}, '
#                               f'VOI-merge-LMC={voi_merge_lmc_all:.6f}, '
#                               f'ARAND-LMC={arand_lmc_all:.6f}')
        
#         print(lmc_all_metrics_str)
#         with open(os.path.join(out_affs, 'lmc_all_scores.txt'), 'w') as f_lmc:
#             f_lmc.write(lmc_all_metrics_str)
#             f_lmc.write('\n')